...
class _ExpenseEditState extends State<ExpenseEdit> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (provider.accountModel != null) {
        ...
        print('Selected Account from API: $selectedAccount');
      }
  ...
  await provider.fetchPaidFormList(); // ✅ Fetch API data before showing dialog
                ...
                children: [
                        ...
                        InkWell(
                          ...
                          child: Padding(
                            ...
                            child: CircleAvatar(
                              radius: 10,
                              backgroundColor: Colors.grey.shade100,
                              child: const Icon(Icons.close, size: 18, color: Colors.green),
                            ),
                          ),
                        ),
                      ],
                  ...
                  SizedBox(
                    ...
                    child: provider.isLoading
                        ...
                        : CustomDropdownTwo(
                            hint: '',
                            items: provider.paidFormList.map((e) => e.accountName).toList(),
                            width: double.infinity,
                            height: 30,
                            labelText: 'Paid To',
                            selectedItem: selectedPaidTo,
                  ...
                  Row(
                    ...
                    children: [
                      TextButton(
                        onPressed: () {
                          if (selectedPaidTo != null && amountController.text.isNotEmpty) {
                            // ✅ Update the existing item directly
                            // item.receiptFrom = selectedPaidTo!;
                            // item.note = noteController.text;
                            // item.amount = double.tryParse(amountController.text) ?? 0.0;
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Expense item updated successfully')),
                            );
                          }
                        },
                        child: const Text('Update'),
