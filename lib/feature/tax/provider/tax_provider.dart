...
class TaxProvider with ChangeNotifier {
  ...
  Future<void> fetchTaxes() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print("Failed to load tax data: ${response.statusCode}");
      }
    } catch (e) {
  ...
  try {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      print("Tax created successfully!");
      // Optionally re-fetch list:
      await fetchTaxes();
    } else {
      print("Failed to create tax: ${response.body}");
    }
  } catch (e) {
    print("Error during tax creation: $e");
  }
...
Future<void> deleteTax(int id) async {
  ...
  final url = Uri.parse('https://commercebook.site/api/v1/tax/remove/?id=$id');

  try {
    final response = await http.get(url); // Assuming the delete is a GET request
    if (response.statusCode == 200) {
      _taxList.removeWhere((tax) => tax.id == id);
      notifyListeners();
      print("Tax deleted successfully!");
    } else {
      print("Failed to delete tax: ${response.body}");
    }
  } catch (e) {
